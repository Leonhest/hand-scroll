...
| Screenshot 1 — the gesture | 1280×800 | `assets/screenshots/1-gesture.png` |
| Screenshot 2 — runs in background | 1280×800 | `assets/screenshots/2-background.png` |
| Screenshot 3 — preview & settings | 1280×800 | `assets/screenshots/3-settings.png` |
| Small promo tile | 440×280 | `assets/promo-440x280.png` |

Screenshots are illustrations generated by `assets/screenshots/make.py`. The hands
are Blender renders of a rigged human from [MPFB](https://extensions.blender.org/add-ons/mpfb/)
(MakeHuman's Blender extension, installed separately; its generated content is
CC0). Poses are bone rotations and IK in `assets/screenshots/hands/render_hands.py`.
Render, then composite:

    /Applications/Blender.app/Contents/MacOS/Blender -b -P assets/screenshots/hands/render_hands.py
    python3 assets/screenshots/make.py

Edit the scripts and re-run rather than editing the PNGs. An earlier set with the
hands drawn as tracking skeletons is kept in `assets/screenshots/skeleton/`.
